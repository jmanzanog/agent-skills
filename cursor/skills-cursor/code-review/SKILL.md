---
name: code-review
description: Performs a comprehensive Code Review comparing the current state with the main branch. Use when the user asks for a review, audit, or analysis of their changes.
---
...
1.  **Get Changes**:
    ...
    *   **Windows (PowerShell)**:
        ```powershell
        powershell -ExecutionPolicy Bypass -File "$HOME\.agent\skills\code_review\get_changes.ps1"
        ```
    *   **Linux / macOS (Bash)**:
        ```bash
        bash "$HOME/.agent/skills/code_review/get_changes.sh"
        ```
